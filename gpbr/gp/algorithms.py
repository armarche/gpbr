    ...
    for _ in range(lambda_):
        ...
        if op_choice < cxpb:            # Apply crossover
            try:
                ...
                set_trace()
                raise RuntimeError(f"Crossover failed: {e}") from e
            ...
            try:
                ...
                set_trace()
                raise RuntimeError(f"Mutation failed: {e}") from e
...
def customEaMuPlusLambda(population, toolbox, mu, lambda_, cxpb, mutpb, ngen,
    ...
    *ngen* generations are done, the algorithm returns a tuple with the final
    ...
    # Precalculate the function values
    precalculations = toolbox.map(toolbox.precalculate, invalid_ind)
    for ind, subtrees_vals in zip(invalid_ind, precalculations):
        ind.subtrees_values = subtrees_vals

    # Simplify constant subtrees first
    for ind in invalid_ind:
        ind, simplified = toolbox.simplify_constants(ind)
        if simplified:
             ind.subtrees_values = toolbox.precalculate(ind)
    ...
    for gen in range(1, ngen + 1):
        # Vary the population
        offspring = customVarOr(population, toolbox, lambda_, cxpb, mutpb)
